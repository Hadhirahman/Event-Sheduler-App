...
export default function EventList({ events, error }) {
  ...
  return (
    <div className="space-y-3 p-3 min-w-[30vw] ">
      {events.map((ev) => (
        <div key={ev._id} className="card rounded-lg shadow-sm">
          <div className="card-body flex flex-col md:flex-row md:items-center">
            ...
            {userData && isAdmin && (
              <div className="mt-3 md:mt-0 md:ml-4 flex items-center gap-2">
                <Link to={`/update/${ev._id}`}>
                  <button className="badge bg-blue-600">edit</button>
                </Link>
                <button
                  className="badge bg-red-600"
